...
document.addEventListener('DOMContentLoaded', () => {
  ...
  cb.addEventListener('change', async () => {
    ...
    try {
      await fetch('update_mingguan.php', {
        ...
        body: new URLSearchParams({ id, bulan, tahun, minggu, status })
      });

      // 🔧 update tampilan total hanya di bulan yang sama
      const card = cb.closest('.card'); 
      const semuaMinggu = card.querySelectorAll(`.bayar-checkbox[data-id="${id}"]`);
      const belumBayar = Array.from(semuaMinggu).filter(x => !x.checked).length;

      // cari baris di bulan ini (bukan semua bulan)
      const row = cb.closest('tr');
      if (row) {
        const totalCell = row.querySelector('td:nth-last-child(2)'); // kolom total
        if (belumBayar === 0) {
          totalCell.innerHTML = `<span class="text-success fw-bold">Lunas</span>`;
        } else {
          totalCell.innerHTML = `<span class="text-danger">Rp ${(belumBayar * 5000).toLocaleString('id-ID')}</span>`;
        }
      }
  ...
  if (btnTambah && modalTambahEl && formTambah) {
    ...
    btnTambah.addEventListener('click', () => { formTambah.reset(); modalTambah.show(); });
    formTambah.addEventListener('submit', e => {
      e.preventDefault();
      fetch('simpan_siswa.php',{method:'POST',body:new FormData(formTambah)})
        .then(res => res.text())
        .then(t => { if(t.includes('ok')) location.reload(); else alert(t); });
    });
  }
  ...
  document.querySelectorAll('.btnEdit').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const data = await fetch('get_siswa.php?id=' + id).then(r=>r.json());
  ...
  document.getElementById('formEdit')?.addEventListener('submit', e => {
    e.preventDefault();
    fetch('update_siswa.php',{method:'POST',body:new FormData(e.target)})
      .then(res=>res.text())
      .then(t=>{ if(t.includes('ok')) location.reload(); else alert(t); });
  });
    ...
    btn.addEventListener('click', () => {
      ...
      if(confirm('Yakin ingin menghapus siswa?')) {
        ...
        fetch('delete_siswa.php',{method:'POST',body:fd})
          .then(res=>res.text()).then(t=>{ if(t.includes('ok')) location.reload(); else alert(t); });
      }
    });
  });
});
